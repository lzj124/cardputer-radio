...
public:
    ...
    void drawSearchInput(const char* query, unsigned long tickMs) {
        ...
        if ((tickMs / BLINK_INTERVAL) % 2 == 0) {
            int cx = 6 + (dlen < maxChars ? dlen : maxChars) * 6;
